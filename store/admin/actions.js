...
import {
  ...
  SET_PROJECTS,
  SET_SCOPES,
  SET_SKILLS, REMOVE_CATEGORY_BY_ID, SET_CATEGORIES, SET_TYPES, REMOVE_TYPE_BY_ID, SET_SELECTED_SKILL
...
export const GET_SKILL_BY_ID = 'admin/getSkillById';
export const CREATE_SKILL = 'admin/createSkill';
export const UPDATE_SKILL = 'admin/updateSkill';
export const UPDATE_SKILL_DISPLAY_INDEX = 'admin/updateSkillDisplayIndex';
...
export const UPDATE_PROJECT_DISPLAY_INDEX = 'admin/updateProjectDisplayIndex';
...
export default {
  ...
  async updateProjectDisplayIndex({commit}, data) {
    try {
  ...
  async getSkillById({commit}, id) {
    try {
      let response = await this.$axios.get(`/skills/${id}`)
      if (apiHandler.isSuccess(response.status)) {
        commit(SET_SELECTED_SKILL, response.data.data.skill);
        commit(SET_SCOPES, response.data.data.scopes);
        return true
      }
      return false
    } catch (e) {
  ...
  async updateSkill({commit}, data) {
    try {
      let response = await this.$axios.post('/skills/update', data)
      if (apiHandler.isSuccess(response.status)) {
        commit(SET_SUCCESS_MESSAGE, undefined, {root: true})
        return true
      }
  ...
  async updateSkillDisplayIndex({commit}, skills) {
    try {
      let response = await this.$axios.post('/skills/update/display-index', skills)
      if (apiHandler.isSuccess(response.status)) {
        commit(SET_SUCCESS_MESSAGE, undefined, {root: true})
        return true
      }
