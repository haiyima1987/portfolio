    ...
    <FormWrapper :send-form-data="updateDisplayIndex" class="project-list-form">
      <DragList :drag-items="domProjects"
        ...
        <template v-slot:item="{ item }">
          <h5 class="title-project">{{ item.project.name }}</h5>
...
<script>
  ...
  import {DELETE_PROJECT, GET_PROJECTS, UPDATE_DISPLAY_INDEX} from "../../../store/admin/actions";
  ...
  export default {
    ...
    methods: {
      async updateDisplayIndex () {
        ...
        // send request
        await this.$store.dispatch(UPDATE_DISPLAY_INDEX, updatedIndexes)
      },
    ...
    watch: {
      projects: function (newVal) {
        this.domProjects = newVal.map(project => new DomProject(project.id, project))
      }
    },
